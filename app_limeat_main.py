...
# Import the necessary function from main.py
from main import get_ai_suggestions

# Inside your existing MealAnalyzer class, modify or add logic to use the `get_ai_suggestions` function.
...
class MealAnalyzer:
    ...
    def analyze_meal_image(self, image_data):
        ...
        try:
            ...
            # Get AI suggestions from the meal ingredients
            ingredients = [ingredient['nom'] for ingredient in result['ingredients']]
            ai_suggestions = get_ai_suggestions(ingredients)
            
            # Include AI suggestions into the result
            if ai_suggestions:
...
def display_results(result, analyzer):
    ...
    try:
        ...
        if 'ai_suggestions' in result:
            ...
            st.write(f"**Nom du plat suggéré :** {ai_suggestions['dish_name']}")
            st.write("**Ajouter pour augmenter les calories :**")
            st.write(", ".join(ai_suggestions['add_calories']))
            
            st.write("**Retirer pour diminuer les calories :**")
            st.write(", ".join(ai_suggestions['remove_calories']))
            
            st.write("**Ajouter pour améliorer la Nutri-Score :**")
            st.write(", ".join(ai_suggestions['add_health']))
            
            st.write("**Retirer pour améliorer la Nutri-Score :**")
            st.write(", ".join(ai_suggestions['remove_health']))
